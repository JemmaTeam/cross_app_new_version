...
Widget buildPostcodeInput(
    ...
    Function(String?) onSave) {  // 保存函数，用于处理表单提交的数据
  return Form(
    key: formKey, // 设置表单键
    child: Container(
      constraints: const BoxConstraints(maxWidth: 200), // 设置容器的最大宽度
      padding: const EdgeInsets.symmetric(vertical: 5, horizontal: 5), // 设置容器内边距
      decoration: BoxDecoration(
...
String? _validatePostcode(String? value) {
  ...
  if (!_isValidAustralianPostcode(value)) return 'Invalid postal code';  // Check if it's a valid Australian postcode
  return null;
}
